...
\begin{flushleft}
	...
	You are given an \textbf{odd} integer $n$ along with a sequence $a_1, a_2, \cdots, a_n$. There is \textbf{only one} unique element occurs once. Your task is to find it.
...
\begin{flushleft}
	...
	Now Patricky has collected $n$ each of $n$ flavors of coffee, he wants to arrange them into a $n$ $\times$ $n$ matrix so that \textbf{NO} two same flavors are next to each other. To make this matrix more interestring, initially in each row there is one coffee placed in the column 1.
